      ...
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            disabled={disabled}
            className={`border-[3px] border-black px-3 py-1.5 text-xs font-black uppercase transition-all disabled:opacity-50 disabled:pointer-events-none ${
              isSelected
...
function HowItWorks() {
  return (
    <div className={`mt-10 flex flex-col ${WIDTH} gap-4`}>
      <div className="flex items-center gap-2">
        <span className="bg-black px-2.5 py-0.5 text-xs font-black uppercase text-white">
          ✦ GUÍA RÁPIDA
        </span>
        <span className="text-xs font-bold uppercase text-black/60">¿CÓMO DESCARGAR?</span>
      </div>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {HOW_IT_WORKS.map((item) => (
          <div
            key={item.step}
            className={`flex flex-col gap-2 bg-white p-4 ${BOX} transition-transform hover:-translate-y-1`}
          >
            <div className="flex items-center justify-between">
              <span className={`border-[2px] border-black ${item.bg} px-2 py-0.5 text-xs font-black`}>
                PASO {item.step}
              </span>
              <item.Icon className="h-5 w-5 text-black" strokeWidth={2.5} />
            </div>
            <h3 className="mt-1 text-sm font-black uppercase">{item.title}</h3>
            <p className="text-xs font-bold leading-relaxed text-black/70">{item.desc}</p>
          </div>
        ))}
      </div>
    </div>
...
function App() {
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-between text-black">
        ...
        <main className="flex w-full flex-col items-center px-4 py-12">
            ...
            <div className="relative flex-1">
              ...
              {url && (
                <button
                  ...
                  disabled={busy}
                  aria-label="Limpiar"
                  className="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center border-[3px] border-black bg-[#ff5c5c] hover:bg-[#ff7a7a] active:translate-y-[-40%] disabled:opacity-50"
                >
                  <XIcon className="h-4 w-4 text-black" strokeWidth={3} aria-hidden="true" />
                </button>
              )}
            </div>
            <button
          ...
          <div className={`mt-4 flex ${WIDTH} flex-wrap gap-2`}>
            {PLATFORMS.map((p) => {
              ...
              return (
                <span
                  key={p.label}
                  className={`flex items-center gap-1.5 border-[3px] border-black px-3 py-1 text-xs font-black uppercase transition-all duration-150 ${
                    isActive
                      ? `${p.activeBg} shadow-[4px_4px_0_0_#000] scale-105 -translate-y-0.5`
                ...
                >
                  ...
                  {p.label}
                  {isActive && (
                    <span className="ml-1 bg-black px-1.5 py-0.2 text-[10px] font-black text-white">
                      ACTIVO
                    </span>
                  )}
                </span>
          ...
          {loading && (
            <div className={`mt-8 flex ${WIDTH} items-center justify-center gap-3 bg-white p-8 ${BOX}`}>
              ...
              <span className="font-black uppercase">Buscando información...</span>
            </div>
          )}
          ...
          {!loading && videoInfo && (
            <div className={`mt-8 flex flex-col gap-5 ${WIDTH} bg-white p-5 ${BOX}`}>
              ...
              <div className="flex items-center justify-between border-b-[3px] border-black pb-3">
                <div className="flex items-center gap-2">
                  <span className="border-[2px] border-black bg-lime-300 px-2 py-0.5 text-xs font-black uppercase">
                    ✓ DETECTADO
                  </span>
                  <span className="bg-black px-2 py-0.5 text-xs font-black uppercase text-white">
                    {videoInfo.extractor}
                  </span>
                </div>
                <button
                  ...
                  onClick={handleClear}
                  disabled={busy}
                  className={`flex items-center gap-1.5 bg-yellow-300 px-3 py-1 text-xs font-black uppercase hover:bg-yellow-200 ${BOX} ${PRESS}`}
                >
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                {videoInfo.thumbnail && (
                  <div className="relative shrink-0 overflow-hidden border-[3px] border-black bg-black sm:w-44">
                    <img
                    ...
                    {videoInfo.duration != null && (
                      <span className="absolute bottom-1 right-1 flex items-center gap-1 bg-black/90 px-1.5 py-0.5 text-[11px] font-black text-white">
                ...
                <div className="flex min-w-0 flex-1 flex-col justify-center gap-2">
                  <h2 className="line-clamp-2 text-base font-black uppercase leading-tight sm:text-lg">
                    {videoInfo.title}
                  </h2>
                  <div className="flex flex-wrap items-center gap-2">
                    {videoInfo.uploader && (
                      <span className="flex items-center gap-1 border-[2px] border-black bg-yellow-300 px-2 py-0.5 text-xs font-black uppercase text-black">
                    ...
                    {url && (
                      <a
                        ...
                        className="flex items-center gap-1 border-[2px] border-black bg-white px-2 py-0.5 text-xs font-black uppercase hover:bg-gray-100"
                      >
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {downloading === 'video' ? (
                      ...
                      <button
                        ...
                        disabled={busy}
                        className={`flex items-center justify-center gap-2 bg-lime-300 py-3.5 px-4 text-sm font-black uppercase hover:bg-lime-200 ${BOX} ${PRESS}`}
                      >
                    ...
                    {downloading === 'audio' ? (
                      ...
                      <button
                        ...
                        disabled={busy}
                        className={`flex items-center justify-center gap-2 bg-fuchsia-400 py-3.5 px-4 text-sm font-black uppercase hover:bg-fuchsia-300 ${BOX} ${PRESS}`}
                      >
              ...
              {isYouTube && (
                <>
                  {videoInfo.video_qualities.length > 0 && (
                    <div className="flex flex-col gap-2.5 border-t-[3px] border-black pt-4">
                      <div className="flex items-center justify-between">
                        <span className="bg-black px-2.5 py-0.5 text-xs font-black uppercase text-white">
                          📹 OPCIONES DE VIDEO
                        </span>
                      ...
                      {downloading === 'video' ? (
                        ...
                        <button
                          ...
                          onClick={() => handleDownload('video')}
                          disabled={busy}
                          className={`mt-1 flex items-center justify-center gap-2 bg-lime-300 py-3 px-4 text-sm font-black uppercase hover:bg-lime-200 ${BOX} ${PRESS}`}
                        >
                          <Film className="h-4 w-4" strokeWidth={2.5} />
                          Descargar video
                        </button>
                  ...
                  <div className="flex flex-col gap-2.5 border-t-[3px] border-black pt-4">
                    ...
                    {downloading === 'audio' ? (
                      ...
                      <button
                        ...
                        onClick={() => handleDownload('audio')}
                        disabled={busy}
                        className={`mt-1 flex items-center justify-center gap-2 bg-fuchsia-400 py-3 px-4 text-sm font-black uppercase hover:bg-fuchsia-300 ${BOX} ${PRESS}`}
                      >
                        <Headphones className="h-4 w-4" strokeWidth={2.5} />
                        Descargar audio
                      </button>
      ...
      <footer className="w-full border-t-[3px] border-black bg-white py-6 text-center text-xs font-black uppercase">
        <div className="flex flex-col items-center justify-center gap-2 px-4 sm:flex-row sm:gap-6">
